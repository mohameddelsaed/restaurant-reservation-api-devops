...
export class CategoriesService {
  ...
  async remove(id: string) {
    ...
    const activeBookingsCount = await this.bookingRepo.count({
      where: [
        { categoryId: id, status: BookingStatus.HELD },
        { categoryId: id, status: BookingStatus.CONFIRMED },
        { categoryId: id, status: BookingStatus.WAITING },
      ],
    });

    if (activeBookingsCount > 0) {
      throw new BadRequestException('Cannot delete category with active associated bookings');
    }
